...
export type FieldDescriptionType<T extends { [index: string]: any; }> =
    TextFieldDescription<T>
    | DateFieldDescription<T>
    | SelectFieldDescription<T>

// Description of the entire form that is used for building
export interface FormDescription<T extends { [index: string]: any; }> {
...
const createActions = (name: string) => ({
    loading: createCustomAction<string, [boolean], { name: string, isLoading: boolean }>('form/loading', (isLoading) => ({
        name,
        isLoading
    })),
    error: createCustomAction<string, [string], { name: string, errorMessage: string }>('form/error', (errorMessage: string) => ({
        name,
        errorMessage
    }))
})
...
export const formBuilderReducer = createReducer<FormBuilderState, FormActions>({})
    ...
    .handleType('form/loading', (state, action) => ({
        ...state,
        [action.name]: {...state[action.name], isLoading: action.isLoading}
    }))
    ...
    Form: ({handleSubmit, initialState}: FormProps<T>) => {
        ...
        const showError = (name: any) =>
            formState.errors[name] ?
                <div className='field-error-msg'><IonIcon icon={closeCircle} color='red'/> {formState.errors[name]}
                </div>
                : undefined
        ...
        const createDateField = <T extends { [index: string]: any }>({def, widgetState}: {
            ...
            const dateProps = Object.entries(def).reduce((acc, [key, value]) => value && datePropsNames.includes(key) ? {
                ...acc,
                [key]: value
            } : acc, {})
            return (
        ...
        const fieldCreator = (def: FieldDescriptionType<T>, widgetState: ReturnType<typeof raw>) => {
            switch (def.type) {
                case 'date':
                    return createDateField({def: (def as DateFieldDescription<T>), widgetState});
                case 'select':
                    return createSelectField({def: (def as SelectFieldDescription<T>), widgetState});
                default:
                    return createTextField({def: def as TextFieldDescription<T>, widgetState});
            }
        }
        ...
        const validateForm = () =>
            fields.map(f => f.validators && {
                name: f.label,
                error: validator(f)(formState.values[f.name], formState.values)
            })
        ...
        return <div>
            <form onSubmit={
                ...
                {fields.map(f => fieldCreator(f, raw({
                    name: f.name,
                    onChange: valueExtractor,
                    validate: validator(f)
                })))}
                <IonButton type="submit" expand="block"
                           disabled={!formIsValid()}>{spinner(isLoading)}{submitLabel}</IonButton>
            </form>
            <IonToast
